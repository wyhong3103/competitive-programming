...
struct SegTree{
    ...
    ll valHelper(int k, int l, int x, int lx, int rx){
        if (val[x] < k || rx-1 < l) return -1;
        if (rx-lx == 1){
            return lx; 
        }

        int m = (lx+rx)/2;
        int ret = valHelper(k, l, (x*2)+1, lx, m);
        if (ret == -1){
            ret = valHelper(k, l, (x*2)+2, m, rx);
        }
        return ret;
    }
 
    ll getVal(int k, int l){
        return valHelper(k, l, 0, 0, size);
    }
};
